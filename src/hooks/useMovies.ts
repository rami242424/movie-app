...
export function useMovies(keyword:string){
  ...
  const getSortedMovies = () => {
    ...
    const copyMovies = [...fetchState.data];
    if(filter === "rating") return copyMovies.sort((a,b) => b.vote_average - a.vote_average);
    if(filter === "release") return copyMovies.sort((a,b) => b.release_date.localeCompare(a.release_date));
    return copyMovies;
  }
